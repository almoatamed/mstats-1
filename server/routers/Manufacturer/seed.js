...
const seed = async () => {
  ...
  let unique = is_unique('manufacturers', 'name', name)
  while (!unique) {
    name = await gen.title(2)
    unique = is_unique('manufacturers', 'name', name)
  }
  ...
  const insertion_query = `
    insert into manufacturers (name, address, phone, created_by_user) values(
      '${name}',
      '${address}',
      '${phone}',
      '${created_by_user}'
